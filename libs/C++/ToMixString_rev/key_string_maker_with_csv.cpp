...
bool CheckResult(const picojson::object& result_json)
{
    using namespace std;
    
    std::string mix_string("");
    picojson::array arr({});
    try
    {
        mix_string = result_json.at("mix_string").get<std::string>();
        arr = result_json.at("key_info_list").get<picojson::array>();
    }
 ...
 * @param: 変更予定 ＝＞ 1=文字列、...、
 */
int Csv2KeyJson(picojson::object& dest, char *filepath)
{
    using namespace std;
    ...
    for (auto item : str_list)
    {
        ...
        {
            std::cerr << "key情報取得失敗 : " << e.what() << std::endl;
            return -1;
        }
    }
        
    // Result!
    dest.emplace("mix_string", ret);
    dest.emplace("key_info_list", data_list);
